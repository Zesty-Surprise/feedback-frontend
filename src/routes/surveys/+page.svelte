...
<p slot="header" class="text-[#595959] font-semibold">
  Create a new survey
</p>

<form on:submit|preventDefault={handleSubmit}>
  <div class="flex flex-col mt-8 ml-8 mr-8 gap-6 text-[#595959]">
    ...
    <div class="flex flex-row justify-between items-center">
      ...
      <select class="w-2/4 rounded-md border border-[#2b21181a] pt-1 pb-1" bind:value={selected}>
        {#each options as value}<option class="bg-[#fcf8f4]" {value}>{value}</option>{/each}
      </select>
    </div>
    <div class="pt-3 pb-2 self-center">
      <button class="text-white bg-[#de896e] hover:brightness-90 transition duration-200 focus:outline-none font-medium rounded-full text-md px-6 py-2 text-center" type="submit" on:click={() => (showModal = false)}>
        Create
      </button>
    </div>
  </div>
...
<div class="flex flex-wrap ml-12 mr-16">
  <Survey title="Create a new survey">
      <button class="flex justify-center items-center h-full w-full" on:click={() => (showModal = true)}>
        <div class="bg-[#de896e] rounded-full w-14 h-14 flex justify-center items-center">
