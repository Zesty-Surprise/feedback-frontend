<script lang="ts">
  ...
  const deleteRequest = async (id:string):Promise<void> => {
    const res = await fetch(`https://test.axelzublena.com/api/templates/${id}`, {
        method: "DELETE"
    });
...
<div class="flex justify-center grid grid-cols-4 gap-10 mt-10 px-10 text-zinc-600">
  <div class="rounded-xl text-center h-96 w-46">
    <div class="h-full flex items-center justify-center text-white border-custom">
      <a href="/templates/builder">
        <div
          class="flex items-center justify-center mx-2 "
        >
          <span class="text-color transform hover:scale-150 hover:rotate-90 transition duration-500 ease-in-out ">
  ...
  {#each data.templates as template}
    ...
    <div class="bg-white shadow-md rounded-xl text-center h-96 w-46 border-custom hover:shadow-xl transition duration-250 ease-in-out">
      <div class="flex justify-end px-1 pt-1 border-b-2 border-neutral-200 mb-2">
        ...
        <button on:click={()=>{deleteTemplate(template._id)}}>
          <div
            class="flex items-center justify-center rounded-full h-12 w-12 "
          >
            <span class="font-bold text-2xl ">
