<script lang="ts">
  ...
  const saveTemplate = async (): Promise<void> => {
    ...
    goto(`/templates/${data.template._id}`, { replaceState: true})
  };
...
<div class="grid grid-cols-3 grid-rows-3 gap-3 text-zinc-600 m-10">
    <div class="row-span-4 col-start-1 row-start-1 rounded-xl ml-2">
        ...
        <div class="p-4 bg-white border border-gray-200 rounded-xl shadow m ml-0">
                <div class="h-24 py-1">
                    <label for="text" class="block mb-2 text-sm font-medium text-gray-900 ">Template name</label>
                    <input type="text" bind:value={newTemplate.name} class="w-full rounded-xl" placeholder="New template"/>
                </div>
                <di class="flex">
                    <button type="submit" class="w-18 text-red-400 bg-white  border-4 border-red-400 rounded-xl p-2 mr-2 hover:bg-red-200" on:click={()=>{goto('/templates')}}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 9l-3 3m0 0l3 3m-3-3h7.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                          
                          </svg>
                    </button>
                    <button type="submit" class="w-full bg-white text-green-500 border-4 border-green-500 rounded-xl p-2 text-md font-medium 
                    hover:bg-green-200" on:click={saveTemplate}>
                    Save Template
                  </button>
                </di>
        </div>
    ...
    <div class="col-span-2 row-span-3 mr-10 border border-custom shadow h-full p-4 rounded-xl">
        <div class="rounded-md mb-5">
        ...
        <div class="bg-white border shadow-md hover:shadow-xl transition duration-250 ease-in-out border-gray-200 rounded-xl px-4 py-3 mb-4">
            ...
            <div class="">
                <label for="support" class="text-s mb-2 ">Component <strong>title</strong> or <strong>question</strong></label>
                <textarea id="support" bind:value={component.custom_text}  placeholder="Provide a text for the " class="border rounded-xl w-full h-16"></textarea>
            </div>
        </div>
        {/each}
